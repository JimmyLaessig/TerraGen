...
ShadowMapTechnique::ShadowMapTechnique(QOpenGLFunctions_4_5_Core *functions, int width, int height):
    functions(functions),
    width(width),
    height(height)
{
...
//    functions->glUniform3fv(location, 1, glm::value_ptr(camera->transform.getPosition()));
...
//        functions->glUniformMatrix4fv(location, 1, GL_FALSE, glm::value_ptr(viewProjectionMatrix * tile.getLocalToWorldMatrix()));

//        // Model matrix of the current tile
//        location =  functions->glGetUniformLocation(shader->programId(), "modelMatrix");
//        functions->glUniformMatrix4fv(location, 1, GL_FALSE, glm::value_ptr(tile.getLocalToWorldMatrix()));
